...
/**
 * Represents a quartic Bezier curve (degree 4).
 *
 * A quartic Bezier curve is defined by five control points.
 * This is a higher-order curve that provides more control than cubic curves.
 *
 * @author Simon André <smn.andre@gmail.com>
 */
final readonly class QuarticCurve extends Curve
{
    /**
     * Create a new quartic Bezier curve.
     *
     ...
     * @param Point $p4 Fifth control point (end)
     */
    public function __construct(
        ...
        parent::__construct($p0, $p1, $p2, $p3, $p4);
    }
     ...
     *
     * @return array{0: QuarticCurve, 1: QuarticCurve} Array containing [leftCurve, rightCurve]
     */
    public function split(float $t): array
    {
        ...
        return [
            new QuarticCurve($left[0], $left[1], $left[2], $left[3], $left[4]),
            new QuarticCurve($right[0], $right[1], $right[2], $right[3], $right[4]),
        ];
    }
     ...
     * For quartic, the derivative is a cubic curve.
     *
     * @param float $t Parameter value between 0 and 1
     *
    ...
    {
        [$p0, $p1, $p2, $p3, $p4] = $this->points;

        // Derivative control points (cubic curve)
        $d0 = new Point(4 * ($p1->x - $p0->x), 4 * ($p1->y - $p0->y));
        $d1 = new Point(4 * ($p2->x - $p1->x), 4 * ($p2->y - $p1->y));
        $d2 = new Point(4 * ($p3->x - $p2->x), 4 * ($p3->y - $p2->y));
        $d3 = new Point(4 * ($p4->x - $p3->x), 4 * ($p4->y - $p3->y));

        // Evaluate the derivative curve (cubic) at t
        return $this->deCasteljau([$d0, $d1, $d2, $d3], $t);
    }
